...
**Supabase Migration Initiated for Scalability (August 2025)**: Began comprehensive migration from development PostgreSQL database to Supabase for expansion and scalability requirements. Core endpoints (order creation, payment processing, STL generation, order retrieval) updated with proper field mapping between API camelCase (photoUrl, style, engravingText) and Supabase snake_case (image_url, model_type, engraving_text). Created migration scripts and helper functions for Supabase integration. Current status: Code updated but requires Supabase schema setup (users and orders tables) before full deployment. Payment and STL generation pipeline verified working with proper field transformations.
